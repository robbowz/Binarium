...
declare -r SCRIPT_LOGFILE="/tmp/binarium_node_${DATE_STAMP}_out.log"
declare -r SCRIPTPATH=$( cd $(dirname ${BASH_SOURCE[0]}) > /dev/null; pwd -P )
declare -r WANIP=$(dig +short myip.opendns.com @resolver1.opendns.com)


function print_greeting() {
	echo -e "[0;35m binarium masternode install script[0m\n"
}
...
function swaphack() {
	...
	free -h
	rm -f /var/binarium_node_swap.img
	touch /var/binarium_node_swap.img
	dd if=/dev/zero of=/var/binarium_node_swap.img bs=1024k count=2000 &>> ${SCRIPT_LOGFILE}
	chmod 0600 /var/binarium_node_swap.img
	mkswap /var/binarium_node_swap.img &>> ${SCRIPT_LOGFILE}
	free -h
...
function remove_old_files() {
	...
	sudo killall binariumd
	sudo rm -rf /root/binarium
	sudo rm -rf /root/.binarium
	sudo rm -rf /root/.binariumcore
   	sudo rm -rf binariumd
   	sudo rm -rf binarium-cli
	echo "Done..."
}
...
function configure_masternode() {
	...
	/root/binarium/binarium-cli stop
	echo "It's okay."
	sleep 10
	echo -e "rpcuser=binariumuser\nrpcpassword=${PASSWORD}\rpcallowip=127.0.0.1\nport=${NODEPORT}\nexternalip=${WANIP}\nlisten=1\nmaxconnections=250\bind=${WANIP}:${NODEPORT}" >> ${conffile}
	echo ""
	echo -e "[0;35m==================================================================[0m"
	...
	echo "1 MINUTE LEFT"
	sleep 60
	masternodekey=$(/root/binarium/binarium-cli masternode genkey)
	/root/binarium/binarium-cli stop
	sleep 20
...
#Setting auto start cron job for binariumd
cronjob="@reboot sleep 30 && /root/binarium/binariumd"
crontab -l > tempcron
